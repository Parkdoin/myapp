...
public class GuestBookMgr {
	...
	public void insertGuestBook(GuestBookBean bean) {
		...
		try {
			...
			//DB 실행되는 코드. 리턴값은 적용된 정수의 갯수 - insert은 성공 1이고 실패는 0
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
		} finally {
	...
	//GuestBook List : 방명록 리스트 -> 반드시 로그인 -> 로그인의 id와 등급
	public Vector<GuestBookBean> listGuestBook(String id, String grade){
		...
		try {
			...
			if(grade.equals("1")) {//관리자 로그인
				...
				pstmt = con.prepareStatement(sql);
			}else if(grade.equals("0")) {//일반계정 본인글 모두 + 비밀글 아닌 글
	...
	//Comment List
	public Vector<CommentBean> listComment(int num/*방명록번호*/){
		Connection con = null;
		PreparedStatement pstmt = null;
		ResultSet rs = null;
		String sql = null;
		...
		try {
			...
			while(rs.next()) {
				...
				bean.setCnum(rs.getInt(1));//댓글번호
				bean.setNum(rs.getInt(2));//방명록번호
				bean.setCid(rs.getString(3));//댓글쓴 id
				bean.setComment(rs.getString(4));//댓글
				bean.setCip(rs.getString(5));//댓글 쓴 ip
				bean.setCregDate(SDF_DATE.format(rs.getDate(6)));//댓글 쓴 날짜.
				vlist.addElement(bean);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	//Comment All Delete : 방명록 글삭제시 관련 댓글 모두 삭제
