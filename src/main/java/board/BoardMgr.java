...
public class BoardMgr {
	...
	public static final String SAVEDIRECTORY = "C:/Jsp/eclipse-workspace/myapp/src/main/webapp/board/fileupload/";
	...
	public void updateBoard2(MultipartRequest mulit) {
		Connection con = null;
		PreparedStatement pstmt = null;
		String sql = null;
		try {
			...
			if(filename!=null&&!filename.equals("")) {
				...
				if(tempFile!=null&&!tempFile.equals("")) {
					...
					if(f.exists()) UtilMgr.delete(SAVEDIRECTORY+tempFile);
				}
				int filesize = (int)mulit.getFile("filename").length();
				sql = "update tblBoard set name=?, subject=?, content=?, filename=?, "
						+ "filesize=? where num=?";
				pstmt = con.prepareStatement(sql);
	...
	//Board Reply : 게시물 답변 -> 파일업로드 기능 없음
